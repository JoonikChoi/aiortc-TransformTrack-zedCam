...
def extractFrame(viewer, zed):
    ...
    if viewer.is_available():
        if zed.grab() == sl.ERROR_CODE.SUCCESS:
            ...
            sample_data = np.random.randint(0, 255, (480, 640, 3), dtype=np.uint8)
...
class VideoTransformTrack(MediaStreamTrack):
    ...
    async def recv(self):
        ...
        zedFrame = extractFrame(self.viewer, self.zed)
        print ('ok')
...
async def offer(request):
    ...
    def on_track(track):
        ...
        if track.kind == "audio":
            ...
            pc.addTrack(
                VideoTransformTrack(
                    relay.subscribe(track), transform=params["video_transform"], res=res, zed=zed, viewer=viewer
                )
            )
            if args.record_to:
...
if __name__ == "__main__":
    ...
    #app.router.add_get("/echo", print('echo'))
